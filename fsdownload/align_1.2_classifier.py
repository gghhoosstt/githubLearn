...
class SASProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, self.n_way, "newtrain.txt")), "newtrain")
...
def main():
    ...
    model = mymodel.MyAlign.from_pretrained(args.bert_model,args.n) # 模型位置，记录分类
    ...
    if args.do_train:
        ...
        train_features, _ ,train_sep = convert_examples_to_features(
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_features, test_both_answers, test_sep = convert_examples_to_features(
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            #设置
            model.sepindex = train_sep
            model.batch = args.train_batch_size
            print("train len sepindex", len(model.sepindex))
            ...
            i = 0
            model.sepindex = test_sep
            print("test len sepindex", len(model.sepindex))
            model.batch = args.eval_batch_size
