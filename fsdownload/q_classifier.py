...
def paint_learning_curve(train_acc,test_acc,epoch):
    plt.figure()
    plt.title("dxhsmwqc")
    train_acc = train_acc
    test_acc = test_acc
    ...
    for i in range(epoch):
        x = np.append(x,i)
    plt.plot(x,train_acc,'o-',color='r',label='train score')
    plt.plot(x,test_acc,'o-',color='g',label='test score')
    plt.ylim(0,1)
    plt.legend(loc="best")
    plt.show()
...
def main():
    ...
    # model.to(device)
    model.cuda()
    if args.local_rank != -1:#分布式训练，多机多卡
        try:
    ...
    #加载traindata
    if args.do_train:
        train_features,trainlines = convert_examples_to_features(
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_features,testdata = convert_examples_to_features(
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        valid_features,both_answers = convert_examples_to_features(
        ...
        if output_mode == "classification":
            ...
            # all_valid_label = valid_label_ids.data.cpu().numpy()#把上行的tensor转numpy
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for i, batch in enumerate(pbar):
                batch = tuple(t.cuda() for t in batch)  # t.to(device)把t复制一份上gpu
                input_ids, input_mask, segment_ids, label_ids = batch
                with torch.no_grad():
                    logits = model(input_ids, segment_ids, input_mask, labels=None)
                if output_mode == "classification":
